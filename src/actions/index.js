...
export const requestEmployees = () => async (dispatch) => {
  ...
  try {
    const json = await axios.get('http://localhost:5000/employees')
...
export const createEmployee = (employeeData) => {
  return (dispatch) => {
    axios.post('http://localhost:5000/employees', employeeData).then(() => {
...
export function updateEmployeeStatus(status, employeeId) {
  return axios.patch(`http://localhost:5000/employees/${employeeId}`, {
    status: status,
  })
}
...
export function confirmUpdateEmployeeStatusAction(status, id) {
  return {
    type: employeesActions.default.CONFIRM_UPDATE_EMPLOYEE_STATUS_ACTION,
    payload: { status, id },
  }
}

export function updateEmployeeStatusAction(status, employeeId) {
  return (dispatch) => {
    updateEmployeeStatus(status, employeeId).then((response) => {
      dispatch(confirmUpdateEmployeeStatusAction(status, employeeId))
    })
  }
}
